...
class Ets_abandonedcart extends Module
{
    ...
    {
        if (!$this->isCached('bo-test-mail.tpl', $this->getCacheId('bo-test-mail'))) {
            $this->smarty->assign([
                'action' => $this->context->link->getAdminLink('AdminEtsACMailConfigs'),
            ]);
        }
        return $this->display(__FILE__, 'bo-test-mail.tpl', $this->getCacheId('bo-test-mail'));
    }
...
//            $this->context->controller->addJS(_PS_JS_DIR_ . 'jquery/plugins/timepicker/jquery-ui-timepicker-addon.js');
    ...
    public function doShortCode($content, $campaign_type, CartRule $cart_rule = null, Context $context = null, $id_ets_abancart_reminder = null)
    {
        ...
        if (!in_array($campaign_type, array('bar', 'browser', 'leave'))) {
            $content = $this->regexColor('product_list', $content, array('id_list' => rand(1111, 99999), 'product_list' => $this->doProductSmarty($context->cart->getProducts(true), $context)));
        }
