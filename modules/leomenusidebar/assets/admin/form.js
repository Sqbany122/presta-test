	...
	$.fn.PavMegaMenuList = function(opts) {
		...
		function manageTreeMenu(){
			if($('ol').hasClass("sortable")){
				...
				$('#typesub').click(function(){
					var typesub = $( "select.type_sub option:selected" ).val();
					var text = $(this).val();
					var $this  = $(this);
				 	$(this).val( $(this).data('loading-text') );
					$.ajax({
						type: 'POST',
						url: config.action+"&typesub=1&rand="+Math.random(),
						data : 'type='+typesub+'&updatetypesub=1' 
					}).done( function (msg) {
						 $this.val( msg );
					}	
					);
		...
		this.each(function() {  
	 		$(".quickedit",this).click( function(){  
	 			location.href=config.action+"&id_sbmegamenu="+$(this).attr('rel').replace("id_","");
	 		} );

	 		$(".quickdel",this).click( function(){  
	 			if( confirm(config.confirm_del) ){
	 				location.href=config.action+"&dodel=1&id_sbmegamenu="+$(this).attr('rel').replace("id_","");
	 			}
...
jQuery(document).ready(function(){
    ...
    $(".leomenusidebar td").attr('onclick','').unbind('click');
});