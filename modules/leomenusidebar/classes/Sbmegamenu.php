...
//			$true = false;
            ...
            foreach ($data as $menu) {
                ...
                if ($menu['type'] == 'html') {
                    ...
                    if ($this->hasChild($menu['megamenu_id'])) {
                        $output .= '<li class="parent dropdown nav-item '.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>';
                        $output .= '<a class="dropdown-toggle has-category"  href="'.$this->getLink($menu).'" target="'.$menu['target'].'">';
                        if ($menu['icon_class']) {
                            $output .= '<span class="hasicon menu-icon-class"><span class="'.$menu['icon_class'].'"></span>';
                        } elseif ($menu['image']) {
                            $output .= '<span class="hasicon menu-icon" style="background:url(\''.$this->image_base_url.$menu['image'].'\') no-repeat;">';
    ...
    public function genMegaMenuByConfig($parent_id, $level, $menu, $hascat = false, $typesub = '')
    {
        ...
        if ($hascat) {
            $output = '<li class="'.$menu['menu_class'].' parent  nav-item '.$class.' " '.$this->renderAttrs($menu).'>
                    <a href="'.$this->getLink($menu).'" class="dropdown-toggle has-category"  target="'.$menu['target'].'">';
        } else {
            $output = '<li class="'.$menu['menu_class'].' parent  nav-item '.$class.' " '.$this->renderAttrs($menu).'>
						<a href="'.$this->getLink($menu).'" class="dropdown-toggle"  target="'.$menu['target'].'">';
        }
        if ($menu['icon_class']) {
            $output .= '<span class="hasicon menu-icon-class"><span class="'.$menu['icon_class'].'"></span>';
        } elseif ($menu['image']) {
    ...
    public function genCatBySubTree(&$html, $result, $level, $o_spans, $target, $columns)
    {
        ...
        foreach ($result as $val) {
            ...
            $html .= '<li class="'.$class_li.'  nav-item">';
    ...
    public function renderMenuContent($menu, $level, $typesub)
    {
        ...
        if ($menu['type'] == 'html') {
            $output .= '<li class="'.$menu['menu_class'].'  nav-item" '.$this->renderAttrs($menu).'>
            ...
            if ($this->hasChild($menu['megamenu_id'])) {
                ...
                $output .= '<li class="parent  nav-item dropdown-submenu'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>';
                if ($menu['show_title']) {
