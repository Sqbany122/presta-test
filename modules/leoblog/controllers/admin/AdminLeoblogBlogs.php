...
//            );
    ...
    public function renderForm()
    {
		...
		if ($this->object->thumb) {
            # validate module
            $thumb_img = __PS_BASE_URI__.'modules/leoblog/views/img/b/'.$this->object->thumb;
        } else {
            # validate module
            $thumb_img = '';
        }
    ...
    protected function _uploadImage($image, $image_w = null, $image_h = null)
    {
        $res = false;
        if (is_array($image) && (ImageManager::validateUpload($image, $this->max_image_size) === false) && ($tmp_name = tempnam(_PS_TMP_IMG_DIR_, 'PS')) && move_uploaded_file($image['tmp_name'], $tmp_name)) {
			...
			if (file_exists(_LEOBLOG_BLOG_IMG_DIR_.'b/'.$img_name)) {
				@unlink(_LEOBLOG_BLOG_IMG_DIR_.'b/'.$img_name);
			}
			$image_type = 'jpg';
            ...
            if (ImageManager::resize($tmp_name, _LEOBLOG_BLOG_IMG_DIR_.'b/'.$img_name, (int)$image_w, (int)$image_h)) {
                $res = true;
            }
        }
