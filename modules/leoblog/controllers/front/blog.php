...
class LeoblogblogModuleFrontController extends ModuleFrontController
{
        ...
        if (!empty($post)) {
            ...
            if (isset($this->context->cookie->leocaptch) && $captcha && $captcha == $this->context->cookie->leocaptch) {
                if ($comment->validateFields(false) && $comment->validateFieldsLang(false)) {
                    $comment->save();
                    $error->message = LeoBlogHelper::getInstance()->l('Thanks for your comment, it will be published soon!!!', false, 'blog');
                    $error->error = false;
                } else {
                    # validate module
                    $error->message = LeoBlogHelper::getInstance()->l('An error occurred while sending the comment. Please recorrect data in fields!!!', false, 'blog');
                }
            } else {
                # validate module
                $error->message = LeoBlogHelper::getInstance()->l('An error with captcha code, please try to recorrect!!!', false, 'blog');
            }
    ...
    public function initContent()
    {
        ...
        $order_by = $config->get('listing_order_by', 'id_leoblog_blog');
        $order_way = $config->get('listing_order_way', 'DESC');
        $samecats = LeoBlogBlog::getListBlogs($category->id_leoblogcat, $this->context->language->id, 0, $limit, $order_by, $order_way, array('type' => 'samecat', 'id_leoblog_blog' => $blog->id_leoblog_blog), true);
        ...
        if ($blog->tags) {
            $tagrelated = LeoBlogBlog::getListBlogs($category->id_leoblogcat, $this->context->language->id, 0, $limit, $order_by, $order_way, array('type' => 'tag', 'tag' => $blog->tags), true);
