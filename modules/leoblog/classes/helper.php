...
class LeoBlogHelper
{
    ...
    public static function buildBlog($helper, $blog, $image_w, $image_h, $config)
    {
        # module validation
        ...
        if (isset($blog['thumb']) && $blog['thumb'] != '') {
            $blog['thumb_url'] = $url._LEOBLOG_BLOG_IMG_URI_.'b/'.$blog['thumb'];
            if (!file_exists(_LEOBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_leoblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'])) {
                @mkdir(_LEOBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_leoblog_blog'], 0777);
                @mkdir(_LEOBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_leoblog_blog'].'/'.$image_w.'_'.$image_h, 0777);
                if (ImageManager::resize(_LEOBLOG_BLOG_IMG_DIR_.'b/'.$blog['thumb'], _LEOBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_leoblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'], $image_w, $image_h)) {
                    # validate module
                    $blog['preview_thumb_url'] = _LEOBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_leoblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'];
                }
            }
            $blog['thumb_url'] = $url._LEOBLOG_BLOG_IMG_URI_.'b/'.$blog['thumb'];
            $blog['preview_thumb_url'] = $url._LEOBLOG_CACHE_IMG_URI_.'b/'.$blog['id_leoblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'];
        }
        $params = array(
     ...
     */
    public function l($string, $specific = false, $name = '')
    {
        static $obj;
        if (!$obj)
        {
