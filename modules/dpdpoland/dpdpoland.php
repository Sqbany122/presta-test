...
class DpdPoland extends CarrierModule
{
	...
	public function install()
	{
		...
		foreach (array_keys($shops) as $id_shop) {
			if (!$this->saveCSVRule((int)$id_shop, 'PL', '0', '0.5', '0', '', _DPDPOLAND_STANDARD_ID_, $current_date, $current_date)
				|| !$this->saveCSVRule((int)$id_shop, 'PL', '0', '0.5', '0', '0', _DPDPOLAND_STANDARD_COD_ID_, $current_date, $current_date)
				|| !$this->saveCSVRule((int)$id_shop, 'GB', '0', '0.5', '0', '', _DPDPOLAND_CLASSIC_ID_, $current_date, $current_date)
				|| !$this->saveCSVRule((int)$id_shop, '*', '0', '0.5', '0', '', _DPDPOLAND_CLASSIC_ID_, $current_date, $current_date)
            ) {
                return false;
            }
		}
     ...
     */
	private function saveCSVRule($id_shop, $iso_country, $weight_from, $weight_to, $parcel_price, $cod_price, $id_carrier, $date_add, $date_upd)
	{
	...
	public function savePackageFromPost()
	{
        ...
        if (_DPDPOLAND_PUDO_ID_ == $id_method) {
            if (Tools::isSubmit('dpdpoland_pudo_code')) {
                ...
                $pudoAddress = $this->getPudoAddress($pudoCode, $order->id_customer);
	...
	private function getRecipientAddress($id_address)
	{
		...
		return array(
			...
			'phone' => str_replace(' ', '', $address->phone) ? $address->phone : $address->phone_mobile
		);
	}
	...
	public function hookHeader()
    {
        ...
        $available_pages = array('order', 'order-opc');

        // For PS 1.4 only
        $scriptName = __PS_BASE_URI__.'order.php';
        $step = Tools::getValue('step');

        if (in_array($current_controller, $available_pages) ||
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        $address = $this->getPudoAddress($pudoCode, $order->id_customer);
        if (null == $address) {
            return;
        }
     ...
     * @return Address|null
     */
    protected function getPudoAddress($pudoCode, $idCustomer)
    {
        if (false == $pudoCode) {
            return null;
        }
