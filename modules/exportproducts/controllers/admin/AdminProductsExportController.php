...
class AdminProductsExportController extends ModuleAdminController
{
    ...
    if( $res ){
      foreach( $res as $task ){
        if ($this->_shouldBeExecuted($task) == true) {
          ...
          $this->_runTask($task['export_settings']);
  ...
  private function _runTask( $moduleSettings )
  {
    $automaticLink = Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.basename(_PS_MODULE_DIR_).'/exportproducts/automatic_export.php?settings='.$moduleSettings.'&id_shop_group='.Tools::getValue('id_shop_group').'&id_shop='.Tools::getValue('id_shop').'&id_lang='.Context::getContext()->language->id.'&secure_key='.Configuration::getGlobalValue('GOMAKOIL_PRODUCTS_EXPORT_TASKS_KEY');
