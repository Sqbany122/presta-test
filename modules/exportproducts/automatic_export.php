...
try{
  ...
  if (Tools::getValue('secure_key')) {
    ...
    if ( ( $secureKey === Tools::getValue('secure_key')) || ( Tools::getValue('secure_key') == Configuration::getGlobalValue('GOMAKOIL_PRODUCTS_EXPORT_TASKS_KEY') ) ) {
      ...
      if( is_int($link) ){
        Tools::redirect(Tools::getShopDomain(true, true).__PS_BASE_URI__.basename(_PS_MODULE_DIR_).'/exportproducts/automatic_export.php?settings='.Tools::getValue('settings').'&id_shop='.Tools::getValue('id_shop').'&id_shop_group='.Tools::getValue('id_shop_group').'&id_lang='.Tools::getValue('id_lang').'&secure_key='.Tools::getValue('secure_key').'&limit='.$link);
        die;
      }
...
function sendEmail( $error = false, $link = false )
{
  ...
  foreach ($emails as $users_email){
    ...
    if( $feedTarget == 'ftp' ){
      $mailMessage .= '<div style="margin-bottom: 10px;"><div style="float: left;width: 100px;margin: 2px 10px 2px 0px;"><strong>'.Module::getInstanceByName('ordersexport')->l('Export date:', 'automatic_export').'</strong></div><div style="float: left;margin-top: 2px;"> ' . date('d/m/Y G:i:s') .'</div><div style="clear: both;"></div></div>';
      $mailMessage .= '<div style="margin-bottom: 10px;"><div style="float: left;margin: 2px 10px 2px 0px;"><strong>'.Module::getInstanceByName('ordersexport')->l('Exported file successfully uploaded on your FTP Server!', 'automatic_export').'</strong></div><div style="clear: both;"></div></div>';
    }
    else{
      if( $link ){
