...
{strip}
    <div class="gdpr-consent-tabs gdpr-consent-tabs-v2">
        <div class="div_control_your_privacy">
            ...
            {/if}
            {* HTML comment, no escape necessary *}
            <div class="div_text">
                {$welcomeTabText nofilter}
            </div>
        </div>

        <div class="div_accept_moreinfo">
            <span class="span_moreinfo">{l s='More info' mod='gdprpro'}</span>
            <button type="button" id="accept-all-gdpr" style="background: {if isset($acceptAllBtnBgColor) && $acceptAllBtnBgColor}{$acceptAllBtnBgColor}{else}green{/if}; color:{if isset($acceptAllTextColor) && $acceptAllTextColor}{$acceptAllTextColor}{else}white{/if};">
            ...
            <div class="div_center_area">
                ...
                <ul class="gdpr-consent-tabs-content">
                    {foreach GdprPro::$cookieCategories as $cookieCategory}
                            ...
                            <li data-content="{$cookieCategory}-cookies-tab"
                                ...
                                <div class="div_text">
                                    <p class="cookie_cat_description">
                                        {* HTML comment, no escape necessary *}
                                        {$cookieCategoryDescriptions.$cookieCategory nofilter}
                                    </p>
                                            ...
                                            {if isset($module.enabled) && $module.enabled == 1 && isset($module.category) && $module.category == $cookieCategory}
                                                ...
                                                <tr id="module_{md5($name)}">
                                                    ...
                                                    <td class="td_description">
                                                        <span class="tooltiptext">{$module.description[$langId]|default:""}</span>
                                                        <span class="description">{$module.description[$langId]|default:""}</span>
                                                    </td>
                        ...
                        {/capture}
                        {* HTML comment, no escape necessary *}
                        {if $cookiesInCategory > 0} {$smarty.capture.cookieCategoryTab nofilter} {/if}
                    {/foreach}
                </ul>
            </div>
        </div>
