...
class PayU extends PaymentModule
{
    ...
    public function orderCreateRequestByOrder($payMethod = null)
    {
		...
		// Szukamy w bazie kolejny order_id z tego samego koszyka
//!!!! UWZGLENDNIC TYLKO Z KOSZYKA		
		// DOPISANE Zwiększamy total_paid
		$my_orders = Db::getInstance()->executeS('
			SELECT * FROM ' . _DB_PREFIX_ . 'orders
			WHERE id_cart = '.$this->order->id_cart.' AND id_order != "' . (int)$this->order->id . '"
		');
		
		$add_id_order_id = '';
		$add_total_paid = 0;
		$this->id_orders = array($this->order->id );

        if ((int)$this->order->id != (int)Tools::getValue('id_order')) {
		
			if($my_orders != null && count($my_orders) > 0){
				foreach ($my_orders as $my_order){
					
					$this->id_orders[] = $my_order['id_order'];
					$add_id_order = $add_id_order .' '. $my_order['id_order'];
					$add_total_paid = $add_total_paid + $my_order['total_paid'];
				};
			};
//echo 'KOSZYK';
		};
//echo '<pre>'.(int)Tools::getValue('id_order');
//print_r($my_orders);
//exit;
//		$this->order->id 
        ...
        $ocreq = array(
            ...
            'description' => $this->l('Order: ') . $add_id_order.' '.$this->order->id . ' - ' . $this->order->reference . ', ' . $this->l('Store: ') . Configuration::get('PS_SHOP_NAME'),
            'products' => array(
                array(
                    'quantity' => 1,
                    'name' => $this->l('Order: ') . $add_id_order.' '.$this->order->id . ' - ' . $this->order->reference,
                    'unitPrice' => $this->toAmount($this->order->total_paid + $add_total_paid)
                )
            ),
            ...
            'totalAmount' => $this->toAmount($this->order->total_paid + $add_total_paid),
...
//exit;
     ...
     * @return mixed
     */
    public function addOrderSessionId($status, $idOrder, $idCart, $payuIdOrder, $extOrderId, $id_orders = array())
    {
		if(count($id_orders) > 1 ){
			...
			foreach($id_orders as $id_order){
				if($idOrder == $id_order){
					...
					$sql = 'INSERT INTO ' . _DB_PREFIX_ . 'order_payu_payments (id_order, id_cart, id_session, ext_order_id, status, create_at, id_orders)
						VALUES (' . (int)$idOrder . ', ' . (int)$idCart . ', "' . pSQL($payuIdOrder) . '", "' . pSQL($extOrderId) . '", "' . pSQL($status) . '", NOW(), "' . pSQL(json_encode($id_orders)) . '")';
					...
					$sql = 'INSERT INTO ' . _DB_PREFIX_ . 'order_payu_payments (id_order, id_cart, id_session, ext_order_id, status, create_at, id_orders)
						VALUES (' . (int)$id_order . ', ' . (int)$idCart . ', "' . pSQL('ZORDER'.$idOrder) . '", "' . pSQL('ZORDER'.$idOrder) . '", "' . pSQL($status) . '", NOW(), "' . pSQL(json_encode($id_orders)) . '")';
    ...
    {
		// POTWIERDZAMY POWIĄZANE ZAMUWIENIA
		if(count($this->id_orders) > 1){
				
			foreach($this->id_orders as $id_order_s){
					
				$sql = 'UPDATE ' . _DB_PREFIX_ . 'order_payu_payments
					SET id_order = "' . (int)$id_order_s . '", status = "' . pSQL($status) . '", update_at = NOW()
					WHERE id_order="' . (int)$id_order_s . '" AND status != "' . OpenPayuOrderStatus::STATUS_COMPLETED . '" AND status != "' . pSQL($status) . '"';
                    ...
                    if (!$withoutUpdateOrderState && $order_state_id != (int)Configuration::get('PAYU_PAYMENT_STATUS_COMPLETED')) {
						...
						if(count($this->id_orders) > 1){
							...
							foreach($this->id_orders as $id_order_s){
								...
								$history = new OrderHistory();
								$history->id_order = $id_order_s;
								
								$order = new Order($id_order_s);
								
								$history->changeIdOrderState(Configuration::get('PAYU_PAYMENT_STATUS_COMPLETED'), $id_order_s);
								$history->addWithemail(true);
								$this->addTransactionIdToPayment($order, $this->getTransactionId($payu_properties));
							}
						} else {
