            ...
            foreach ($this->context->cart->getProducts() AS $key => $value)
            {
                ...
                else
                {
                    if (Configuration::get('FBPIXEL_ATTRID') == 2)
                    {
                        for ($x = 0; $x < $value['quantity']; $x++)
                        {
                            $content_ids .= "'" . Configuration::get('FBPIXEL_PREFIX', $this->context->language->id) . "{$value['id_product']}".Configuration::get('FBPIXEL_SEPSIGN','')."{$value['id_product_attribute']}" . Configuration::get('FBPIXEL_SUFIX', $this->context->language->id) . "',";
                        }
                    }
                    else
                    {
