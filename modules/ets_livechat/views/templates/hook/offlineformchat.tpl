...
<div class="translatable-field lang-{$language->id|intval}" {if $language->id != $defaultFormLanguage}style="display:none"{/if} >
    <div class="chatbox_demo_backend lc_chatbox lc_admin_offline{if $isRTL} lc_chatbox_rtl{/if}{if $config.ETS_LC_HIDE_ON_MOBILE} lc_hide_on_mobile{/if} lc_{$config.ETS_CLOSE_CHAT_BOX_TYPE|escape:'html':'utf-8'}{if !$config.ETS_LC_DISPLAY_AVATA} no_display_avata {/if} {if !$config.ETS_LC_DISPLAY_TIME}no_display_datetime{/if}{if !$config.ETS_LC_DISPLAY_COMPANY_INFO} no_display_compay_info{/if}">
        <div class="lc_heading lc_heading_offline" style="background-color: {if $config.ETS_LC_HEADING_COLOR_OFFLINE}{$config.ETS_LC_HEADING_COLOR_OFFLINE|escape:'html':'utf-8'}{else}#292929{/if};">
            <div class="lc_offline_heading">{$config.ETS_LC_TEXT_HEADING_OFFLINE|escape:'html':'utf-8'}</div>
        ...
        <div class="lc_company_info{if $config.ETS_LC_DISPLAY_COMPANY_INFO=='general'} display_company_info{/if}{if !$config.ETS_LC_DISPLAY_COMPANY_INFO} lc_hide{/if}">
            <div class="company_info">
                ...
                <div class="company company-name">{$config.ETS_LC_COMPANY_NAME|escape:'html':'UTF-8'}</div><span class="title_class" title="{l s='Offline' mod='ets_livechat'}">{l s='Offline' mod='ets_livechat'}</span>
            </div>
            <div class="employee_info company_info">
                ...
                <div class="company-name">{$employee_info.name|escape:'html':'UTF-8'}</div><span class="title_class" title="{l s='Offline' mod='ets_livechat'}">{l s='Offline' mod='ets_livechat'}</span>
            </div>
        ...
        <div class="lc_text_area_demo {if !$config.ETS_LC_ENABLE_EMOTION_ICON || !$emotions}no_display_emotion{/if} {if $config.ETS_DISPLAY_SEND_BUTTON}show_send_box{/if} start_chating {if isset($needCaptcha) && $needCaptcha}show_capcha{/if}">
            ...
            <div class="lc_send_box">
                <input class="btn btn-primary lc_send_offline"{if $config.LC_BACKGROUD_COLOR_BUTTON} style="background:{$config.LC_BACKGROUD_COLOR_BUTTON|escape:'html':'utf-8'}"{/if} type="button" value="{$config.ETS_LC_TEXT_SEND_OffLINE|escape:'html':'utf-8'}" />
            </div>
        </div>
    </div>
</div>
