...
<div class="translatable-field lang-{$language->id|intval}" {if $language->id != $defaultFormLanguage}style="display:none"{/if} >
    <div class="chatbox_demo_backend lc_chatbox lc_admin_do_not_disturb{if $isRTL} lc_chatbox_rtl{/if}{if $config.ETS_LC_HIDE_ON_MOBILE} lc_hide_on_mobile{/if} lc_{$config.ETS_CLOSE_CHAT_BOX_TYPE|escape:'html':'utf-8'}{if !$config.ETS_LC_DISPLAY_AVATA} no_display_avata {/if} {if !$config.ETS_LC_DISPLAY_TIME}no_display_datetime{/if}{if !$config.ETS_LC_DISPLAY_COMPANY_INFO} no_display_compay_info{/if}">
        <div class="lc_heading lc_heading_do_not_disturb" style="background-color: {if $config.ETS_LC_HEADING_COLOR_BUSY}{$config.ETS_LC_HEADING_COLOR_BUSY|escape:'html':'utf-8'}{else}#ff0000{/if};">
            <div class="lc_do_not_disturb_heading">{$config.ETS_LC_TEXT_HEADING_BUSY|escape:'html':'utf-8'}</div>
        ...
        <div class="lc_company_info{if $config.ETS_LC_DISPLAY_COMPANY_INFO=='general'} display_company_info{/if}{if !$config.ETS_LC_DISPLAY_COMPANY_INFO} lc_hide{/if}">
            <div class="company_info">
                ...
                <div class="company company-name">{$config.ETS_LC_COMPANY_NAME|escape:'html':'UTF-8'}</div><span class="title_class" title="{l s='Busy' mod='ets_livechat'}">{l s='Busy' mod='ets_livechat'}</span>
            </div>
            <div class="employee_info company_info">
                ...
                <div class="company-name">{$employee_info.name|escape:'html':'UTF-8'}</div><span class="title_class" title="{l s='Busy' mod='ets_livechat'}">{l s='Busy' mod='ets_livechat'}</span>
            </div>
