...
class CmsController extends CmsControllerCore
{
    public function init()
    {
        if (Configuration::get('purls_cms') == 1)
        {
            ...
            if ($link_pattern)
            {
                $sql = "SELECT tl.id_cms FROM " . _DB_PREFIX_ . "cms_lang tl LEFT OUTER JOIN " . _DB_PREFIX_ . "cms_shop t ON (t.id_cms = tl.id_cms) WHERE tl.link_rewrite='" . $link_pattern . "' AND tl.id_lang=" . (int)$this->context->language->id . " AND t.id_shop=" . (int)$this->context->shop->id;
            ...
            {
                $sql = "SELECT id_cms_category FROM " . _DB_PREFIX_ . "cms_category_lang WHERE link_rewrite='" . $cms_category_link_pattern . "' AND id_lang=" . (int)$this->context->language->id;
