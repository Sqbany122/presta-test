...
class productStatus extends Module
{
    ...
    public function install()
    {
        ...
        $res = $db->executeS("SHOW COLUMNS FROM `"._DB_PREFIX_."product_status_dates` WHERE `Field` = 'tracking_url'");
        if (count($res) == 0) {
            $db->execute("ALTER TABLE `"._DB_PREFIX_."product_status_dates` ADD COLUMN `tracking_url` varchar(255) DEFAULT NULL");
        }
        $res2 = $db->executeS("SHOW COLUMNS FROM `"._DB_PREFIX_."product_status_dates` WHERE `Field` = 'comment'");
        if (count($res2) == 0) {
            $db->execute("ALTER TABLE `"._DB_PREFIX_."product_status_dates` ADD COLUMN `comment` varchar(255) DEFAULT NULL");
        }
    ...
    public function validateField($field, $afterField)
    {
        $res = $this->db->executeS("SHOW COLUMNS FROM `".$this->table."` WHERE `Field` = '".$field."'");
        if (count($res) == 1) return true;
        $this->db->execute("ALTER TABLE `".$this->table."` ADD COLUMN `".$field."` VARCHAR(255) NULL ".($afterField == null ? "" : "AFTER `".$afterField."`")."");
    ...
    */
    ...
    public static function setStatus($id_details, $id_state, $id_employee = 0, $id_lang = 1)
    {
        ...
        self::compliteOrderProductsStatuses($id_details, $id_employee);
    ...
    public static function compliteOrderProductsStatuses($id_order_detail, $id_employee = 0)
    {
        ...
        {
            self::setOrderStatus($result, $Order->id, $id_employee);
        }
    }
    ...
    {
        $db = Db::getInstance();
        $dates = $db->ExecuteS("SELECT `supplier_delivery`, `scheduled_shipping`, `tracking_url`, `comment`  FROM `"._DB_PREFIX_."product_status_dates` WHERE `id_order_detail` = ".$id_order_detail."");
