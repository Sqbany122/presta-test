...
class ProductSearch
{
    ...
    public static function find($id_lang, $conds, $page_number = 1, $page_size = 1, $order_by = 'position', $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        if (!$context) {
        ...
        $score_array = array();
        $words = explode(' ', ProductSearch::sanitize($conds['query'], $id_lang, false, $context->language->iso_code));
		...
		'.$sql_groups.($conds['cate'] ? ' AND c.`id_category` = '.$conds['cate'] : ''));
        //($conds["manu"]? ' AND p.`id_manufacturer` = '. $conds["manu"] : ''));
        ...
        if ($ajax) {
            $sql = 'SELECT DISTINCT p.id_product, pl.name pname,  cl.name cname, i.id_image,
                    ...
                    ($conds['cate'] ? $conds['cate'] : 'product_shop.`id_category_default`').
            ...
            $tempResults = $db->executeS($sql);
            $results = array();
            foreach ($tempResults as $row) {
                $row['image_link'] = Context::getContext()->link->getImageLink($row['prewrite'], $row['id_image'], 'small'.'_default');
                $results[] = $row;
            }
            return $results;
        }
    ...
    public static function indexation($full = false, $id_product = false)
    {
        ...
        while (($products = Search::getProductsToIndex($total_languages, $id_product, 50, $weight_array)) && ($count_product > 0)) {
            ...
            foreach ($products as $product) {
                ...
                foreach ($product as $key => $value) {
                    if (strncmp($key, 'id_', 3) && isset($weight_array[$key])) {
                        $words = explode(' ', ProductSearch::sanitize($value, (int)$product['id_lang'], true, $product['iso_code']));
                        foreach ($words as $word) {
                            if (!empty($word)) {
    ...
    protected static function saveIndex(&$queryArray3)
    {
        if (count($queryArray3)) {
            $sql = 'INSERT INTO '._DB_PREFIX_.'search_index (id_product, id_word, weight)
