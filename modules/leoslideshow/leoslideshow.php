...
class LeoSlideshow extends Module
{
    ...
    {
        $this->context->controller->addJS(__PS_BASE_URI__.'modules/leoslideshow/'.'views/js/jquery-validation-1.9.0/jquery.validate.js');
        $this->context->controller->addJS(__PS_BASE_URI__.'modules/leoslideshow/views/js/admin/show.js');
        $this->context->controller->addCSS(__PS_BASE_URI__.'modules/leoslideshow/views/css/jquery-validation-1.9.0/screen.css');
...
//			unset($layers['3']);
            ...
            if ( LeoSlideshowConfig::getPermission() == false )
            {
                ...
                if( Tools::isSubmit('id_group') == false){
                    ...
                    if($number_group >= 1){
                        ...
                        $errors[] = '<a href="http://apollotheme.com/products/ap-slideshow-prestashop-module/" target="_blank">Click here to buy Ap Slideshow Prestashop Module</a>';
                    }
                }
            }
        }
    ...
    private function _prepareHook($hook_name)
    {
        ...
        $slider_params['playLabel'] = $this->l('Play');
        $slider_params['pauseLabel'] = $this->l('Pause');
        $slider_params['closeLabel'] = $this->l('Close');
            ...
            if (!Tools::getValue('id_slide')) {
                ...
                if ( LeoSlideshowConfig::getPermission() == false )
                {
                    ...
                    if($number_group >= 2){
                        ...
                        $errors .= $this->l("I'm so sorry, you are using free version of Leo slideshow module, could you please update module to pro version to use this function");
                        $errors .= '<br/><a href="http://apollotheme.com/products/ap-slideshow-prestashop-module/" target="_blank">Click here to buy Ap Slideshow Prestashop Module</a>';
                        $errors .= '</div></div>';
    ...
    public function processHookCallBack($group_id)
    {
        ...
        if (!$res) {
            ...
            return 'If you load this Slideshow (not active) from ApPageBuilder module, please access ApPageBuilder module then delete it';
        }
    ...
    /*
     * Translate for controller in frontend
     * Copy form function l()
     */
    public function l2($string, $specific = false, $name = '')
    {
        if (self::$_generate_config_xml_mode) {
            return $string;
        }

        return Translate::getModuleTranslation($this, $string, ($specific) ? $specific : $name);
    }
}
