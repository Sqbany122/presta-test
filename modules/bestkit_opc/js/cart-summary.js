			...
			else
			{
				...
				if (parseInt(jsonData.summary.products.length) == 0)
				{
					...
					else
					{
						...
						location.reload(); //Bestkit customization
					}
				}
				else
				{
...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		if (typeof(product_list[i].price_without_quantity_discount) != 'undefined')
			initial_price = formatCurrency(parseFloat(product_list[i].price_without_quantity_discount), currencyFormat, currencySign, currencyBlank);
		var current_price = '';
		if (priceDisplayMethod != 0)
			current_price = formatCurrency(parseFloat(product_list[i].price), currencyFormat, currencySign, currencyBlank);
		else
			current_price = formatCurrency(parseFloat(product_list[i].price_wt), currencyFormat, currencySign, currencyBlank);
		...
		if (priceDisplayMethod != 0)
		{
			$('#cart_block_product_'+key_for_blockcart+' span.price').html(formatCurrency(parseFloat(product_list[i].total), currencyFormat, currencySign, currencyBlank));
			...
			$('#total_product_price_'+product_list[i].id_product+'_'+product_list[i].id_product_attribute+'_'+product_list[i].id_address_delivery).html(formatCurrency(parseFloat(product_list[i].total), currencyFormat, currencySign, currencyBlank));
		}
		else
		{
			$('#cart_block_product_'+key_for_blockcart+' span.price').html(formatCurrency(parseFloat(product_list[i].total_wt), currencyFormat, currencySign, currencyBlank));
			$('#product_price_'+product_list[i].id_product+'_'+product_list[i].id_product_attribute+'_'+product_list[i].id_address_delivery).html(initial_price_text+current_price);
			$('#total_product_price_'+product_list[i].id_product+'_'+product_list[i].id_product_attribute+'_'+product_list[i].id_address_delivery).html(formatCurrency(parseFloat(product_list[i].total_wt), currencyFormat, currencySign, currencyBlank));
		}

		nbrProducts += parseInt(product_list[i].quantity);
		
		//product_list[i].quantity_without_customization
	...
	else
	{
		...
		if (priceDisplayMethod != 0)
			$('#total_discount').html(formatCurrency(parseFloat(json.total_discounts_tax_exc), currencyFormat, currencySign, currencyBlank));
		else
			$('#total_discount').html(formatCurrency(parseFloat(json.total_discounts), currencyFormat, currencySign, currencyBlank));
		...
		$('.cart_discount').each(function(){
			...
			for (i=0;i<json.discounts.length;i++)
			{
				if (json.discounts[i].id_discount == idElmt)
				{
					...
					{
						if (priceDisplayMethod != 0)
							$('#cart_discount_' + idElmt + ' td.cart_discount_price span.price-discount').html(formatCurrency(parseFloat(json.discounts[i].value_tax_exc * -1), currencyFormat, currencySign, currencyBlank));
						else
							$('#cart_discount_' + idElmt + ' td.cart_discount_price span.price-discount').html(formatCurrency(parseFloat(json.discounts[i].value_real * -1), currencyFormat, currencySign, currencyBlank));
	...
	if (priceDisplayMethod != 0)
	{
		$('#cart_block_shipping_cost').html(formatCurrency(parseFloat(json.total_shipping_tax_exc), currencyFormat, currencySign, currencyBlank));
		$('#cart_block_wrapping_cost').html(formatCurrency(parseFloat(json.total_wrapping_tax_exc), currencyFormat, currencySign, currencyBlank));
		$('#cart_block_total').html(formatCurrency(parseFloat(json.total_price_without_tax), currencyFormat, currencySign, currencyBlank));
	} else {
		$('#cart_block_shipping_cost').html(formatCurrency(parseFloat(json.total_shipping), currencyFormat, currencySign, currencyBlank));
		$('#cart_block_wrapping_cost').html(formatCurrency(parseFloat(json.total_wrapping), currencyFormat, currencySign, currencyBlank));
		$('#cart_block_total').html(formatCurrency(parseFloat(json.total_price), currencyFormat, currencySign, currencyBlank));
	}

	$('#cart_block_tax_cost').html(formatCurrency(parseFloat(json.total_tax), currencyFormat, currencySign, currencyBlank));
	$('.ajax_cart_quantity').html(nbrProducts);

	// Cart summary
	$('#summary_products_quantity').html(nbrProducts+' '+(nbrProducts > 1 ? txtProducts : txtProduct));
	if (priceDisplayMethod != 0)
		$('#total_product').html(formatCurrency(parseFloat(json.total_products), currencyFormat, currencySign, currencyBlank));
	else
		$('#total_product').html(formatCurrency(parseFloat(json.total_products_wt), currencyFormat, currencySign, currencyBlank));
	$('#total_price').html(formatCurrency(parseFloat(json.total_price), currencyFormat, currencySign, currencyBlank));
	$('#total_price_without_tax').html(formatCurrency(parseFloat(json.total_price_without_tax), currencyFormat, currencySign, currencyBlank));
	$('#total_tax').html(formatCurrency(parseFloat(json.total_tax), currencyFormat, currencySign, currencyBlank));

	if (json.total_shipping > 0)
	{
		if (priceDisplayMethod != 0)
		{
			$('#total_shipping').html(formatCurrency(parseFloat(json.total_shipping_tax_exc), currencyFormat, currencySign, currencyBlank));
		}
		else
		{
			$('#total_shipping').html(formatCurrency(parseFloat(json.total_shipping), currencyFormat, currencySign, currencyBlank));
		}
	}
	else
	{
		$('#total_shipping').html(freeShippingTranslationOPC);
	}

	if (json.free_ship > 0 && !json.is_virtual_cart)
	{
		$('.cart_free_shipping').fadeIn();
		$('#free_shipping').html(formatCurrency(parseFloat(json.free_ship), currencyFormat, currencySign, currencyBlank));
	}
	else
	...
	{
		$('#total_wrapping').html(formatCurrency(parseFloat(json.total_wrapping), currencyFormat, currencySign, currencyBlank));
