...
if (!class_exists("LeoFrameworkHelper")) {
    ...
    class LeoFrameworkHelper
    {
            ...
            if ($dirs) {
                ...
                foreach ($dirs as $dir) {
                    ...
                    if (file_exists($dir.'/icon.png')) {
                        $output[$i]['icon'] = _PS_BASE_URL_SSL_._THEME_DIR_.'css/skins/'.basename($dir).'/icon.png';
                    }

                    $skinFileUrl = _PS_BASE_URL_._THEME_DIR_.'css/skins/'.basename($dir).'/';
                    $skinFileUrl = str_replace('http:','', $skinFileUrl);
                    $skinFileUrl = str_replace('https:','', $skinFileUrl);
        ...
        public static function leoEditColumn($table_name, $col_name, $data_type)
        {
            ...
            if (!empty($column)) {
                $sql = 'ALTER TABLE `'._DB_PREFIX_.$table_name.'` MODIFY `'.$col_name.'` '.$data_type;
                $res = Db::getInstance()->execute($sql);
            }
        }
    }
