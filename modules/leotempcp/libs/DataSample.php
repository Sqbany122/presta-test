            ...
            if (file_exists(_PS_ALL_THEMES_DIR_.$this->_theme_dir.'samples/'.$module.'.xml')) {
                ...
                if (isset($content->configurations) && $content->configurations)
                    foreach ($content->configurations->children() as $contentConfig) {
						//DONGND:: fix data sample for leocustomajax
                        if($module == "leocustomajax")
						{
                ...
                # clear data
                if($module == 'appagebuilder') {
                    $id_shop = Context::getContext()->shop->id;
                    $str_id = '-9';
                    $sql = 'SHOW TABLES LIKE \''._DB_PREFIX_.'appagebuilder_positions_shop\'';
                    if (count(Db::getInstance()->executeS($sql))){

                        $sql = 'SELECT id_appagebuilder_positions FROM `'._DB_PREFIX_.'appagebuilder_positions_shop` WHERE id_shop =' . $id_shop;
                        $ids  = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
                        foreach ($ids as $key => $row) {
                            $str_id .= ',' . $row['id_appagebuilder_positions'];
                        }

                        $sql = 'DELETE FROM `'._DB_PREFIX_.'appagebuilder_positions_shop` WHERE id_appagebuilder_positions IN ('.$str_id.')';
                        Db::getInstance()->execute($sql);
                    }
                    $sql = 'SHOW TABLES LIKE \''._DB_PREFIX_.'appagebuilder_positions\'';
                    if (count(Db::getInstance()->executeS($sql))){
                        $sql = 'DELETE FROM `'._DB_PREFIX_.'appagebuilder_positions` WHERE id_appagebuilder_positions IN ('.$str_id.')';
                        Db::getInstance()->execute($sql);
                    }
                }
                //insert data
