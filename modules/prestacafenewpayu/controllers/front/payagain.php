...
if($ip == '213.156.108.94'){
	
	echo '<pre>';
	echo (int)Tools::getValue('id_order');
	exit;
};
*/
...
//		} else {
			...
			$orders = array();
			$order_ids = Order::getOrdersByCartId($cart->id);
...
//            PayUTools::resetSurchargeProductPrice($surcharge_product, $surcharge);
        ...
        if ($can_proceed) {
            try {
                $last_payment = $this->cancelLastPayment($orders[0]);
                ...
                PrestaCafeNewPayu::addLog(
                    "payagain: cancel payment for order #$orders[0]->id failed: ".$e->getMessage(),
                    2
                );
                $payu_error = true;
                $can_proceed = false;
        ...
        if (!$can_proceed) {
            ...
            $customizedDatas = Product::getAllCustomizedDatas((int)$orders[0]->id_cart);
            $id_order_state = (int)$orders[0]->getCurrentState();
            $order_status = new OrderState((int)$id_order_state, (int)$orders[0]->id_lang);
			...
			foreach($orders as $order){
				...
				$total_discounts_tax_incl = $total_discounts_tax_incl + $order->total_discounts_tax_incl;
				$total_discounts_tax_excl = $total_discounts_tax_excl + $order->total_discounts_tax_excl;
				$total_paid = $total_paid + $order->total_paid;
				$total_paid_tax_incl = $total_paid_tax_incl + $order->total_paid_tax_incl;
				$total_paid_tax_excl = $total_paid_tax_excl + $order->total_paid_tax_excl;
				$total_paid_real = $total_paid_real + $order->total_paid_real;
				$total_products = $total_products + $order->total_products;
				$total_products_wt = $total_products_wt + $order->total_products_wt;
				$total_shipping = $total_shipping + $order->total_shipping;
				$total_shipping_tax_incl = $total_shipping_tax_incl + $order->total_shipping_tax_incl;
				$total_shipping_tax_excl = $total_shipping_tax_excl + $order->total_shipping_tax_excl;
				$carrier_tax_rate = $carrier_tax_rate + $order->carrier_tax_rate;
				$total_wrapping = $total_wrapping + $order->total_wrapping;
				$total_wrapping_tax_incl = $total_wrapping_tax_incl + $order->total_wrapping_tax_incl;
				$total_wrapping_tax_excl = $total_wrapping_tax_excl + $order->total_wrapping_tax_excl;
			...
			$orders[0]->total_discounts = $total_discounts;
			$orders[0]->total_discounts_tax_incl = $total_discounts_tax_incl;
			$orders[0]->total_discounts_tax_excl = $total_discounts_tax_excl;
			$orders[0]->total_paid = $total_paid;
			$orders[0]->total_paid_tax_incl = $total_paid_tax_incl;
			$orders[0]->total_paid_tax_excl = $total_paid_tax_excl;
			$orders[0]->total_paid_real = $total_paid_real;
			$orders[0]->total_products = $total_products;
			$orders[0]->total_products_wt = $total_products_wt;
			$orders[0]->total_shipping = $total_shipping;
			$orders[0]->total_shipping_tax_incl = $total_shipping_tax_incl;
			$orders[0]->total_shipping_tax_excl = $total_shipping_tax_excl;
			$orders[0]->carrier_tax_rate = $carrier_tax_rate;
			$orders[0]->total_wrapping = $total_wrapping;
			$orders[0]->total_wrapping_tax_incl = $total_wrapping_tax_incl;
			$orders[0]->total_wrapping_tax_excl = $total_wrapping_tax_excl;
            ...
            if (version_compare(_PS_VERSION_, '1.7', '<')) {
                ...
                $this->context->smarty->assign(
                    'order',
                    (new PrestaShop\PrestaShop\Adapter\Order\OrderPresenter())->present($orders[0])
                );
        ...
        $this->processWithOrder($orders[0], $pos_id, $pos_second_key, $currency->iso_code, $cart, $orders);
    }
    ...
    private function processWithOrder(Order $order, $pos_id, $second_key, $iso_currency, $cart, $orders)
    {
