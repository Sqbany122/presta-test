...
{
    $is_column = (int)SensbitInpostTools::dbCheckColumn('sensbitinpost_dispatch', 'status');
    if (!$is_column) {
        SensbitInpostTools::installSql(array(
            'ALTER TABLE `'._DB_PREFIX_.'sensbitinpost_dispatch` ADD `status` VARCHAR(50) NOT NULL;'
        ));
    }
    return true;
}
