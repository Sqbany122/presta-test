...
{
    $is_column = (int)SensbitInpostTools::dbCheckColumn('sensbitinpost_shipment', 'multipack');
    if (!$is_column) {
        SensbitInpostTools::installSql(array('ALTER TABLE `'._DB_PREFIX_.'sensbitinpost_shipment` ADD `multipack` int(5) UNSIGNED NOT NULL DEFAULT 1;'));
    }
    return true;
}
