...
class GdprPro extends Module
{
    ...
    public $tabs = array(
        ...
        array(
            'name'              => 'GDPR Custom scripts',
            'class_name'        => 'AdminGdprCustomScripts',
            'visible'           => true,
            'parent_class_name' => 'AdminParentModulesSf',
        ),
        array(
    ...
    public function __construct()
    {
        ...
        $this->module_key = '9b5f05f7dfeeffb5032fb2b495b6ccb4';
    ...
    /**
     * Set the default configuration values for the module
     *
     * @param bool $forceUpdate Forces to update the existing configuration values
     *
     * @throws PrestaShopDatabaseException
     * @throws Exception
     */
    public function setDefaultConfigValues($forceUpdate = false)
    {
        $defaultValues = require 'install/default-values.php';
        foreach ($defaultValues as $key => $value) {
            /**
             * Only insert config key if doesn't exists in the database
             */
            if ($forceUpdate || !GdprProConfig::configExists($key)) {
                /**
        ...
        if ($forceUpdate || !GdprProConfig::configExists(GdprProConfig::MODULES_TO_UNLOAD)) {
            $langs = array();
            foreach (Language::getLanguages(false) as $language) {
    ...
    public function install()
    {
        try {
            ...
            $object->createDatabase();
            $object = new GdprCustomScript();
            $object->createDatabase();
            $object = new DataRequest();
            $object->createDatabase();
    ...
    public function uninstall()
    {
        foreach ($this->tabs as $tabItem) {
            try {
                ...
                $this->_errors[] = "Can't delete module tab: " . $exception->getMessage();
            }
        }
    ...
    public function hookDisplayAdminCustomers($params)
    {
        ...
        if ($result = Db::getInstance()->ExecuteS($sql)) {
            ...
            $helper->table = GdprActivityLog::$definition['table'];
            ...
            return $helper->generateList($result, $columns);
        }
    ...
    {
        if (Tools::isSubmit('submitNewsletter') && Tools::getValue('gdpr_consent_chkbox', false) == '1') {
            \PrestaChamps\GdprPro\Models\ActivityLogFactory::makeNewsletterConsent();
        }

        foreach (GdprProCookie::getInstance()->content as $name => $value) {
            if ($value === 'true') {
                $this->context->smarty->tpl_vars['page']->value['body_classes'][$name . "_on"] = true;
            } else {
                $this->context->smarty->tpl_vars['page']->value['body_classes'][$name . "_off"] = true;
            }
        }
        Media::addJsDef(array(
            'gdprSettings' => array(
                ...
                'under18AlertText'          => Configuration::get(
                    GdprProConfig::UNDER_18_ALERT_TEXT,
                    Context::getContext()->language->id
                ),
                'closeModalOnlyWithButtons' => (bool)Configuration::get(GdprProConfig::CLOSE_MODAL_ONLY_WITH_BUTTONS),
                'showWindow' =>
                    (bool)Configuration::get(GdprProConfig::HIDE_POPUP_FOR_BOTS) &&
                    ((new \Jaybizzle\CrawlerDetect\CrawlerDetect())->isCrawler())
                        ? false
                        : !Context::getContext()->cookie->gdpr_windows_was_opened,
                'isBot' => (new \Jaybizzle\CrawlerDetect\CrawlerDetect())->isCrawler(),
                'doNotTrackCompliance'      => (bool)Configuration::get(GdprProConfig::ENABLE_DO_NOT_TRACK_COMPLIANCE),
                'reloadAfterSave'           => (bool)Configuration::get(GdprProConfig::RELOAD_AFTER_BTN_SAVE),
                'reloadAfterAcceptAll'      => (bool)Configuration::get(GdprProConfig::RELOAD_AFTER_BTN_ACCEPT_ALL),
                'scrollLock'                => (bool)Configuration::get(GdprProConfig::ENABLE_SCROLL_LOCK),
                'acceptByScroll'            => (bool)Configuration::get(GdprProConfig::ENABLE_ACCEPT_BY_SCROLL),
            ),
        ));
        ...
        $return = "";
        try {
            $scriptRegisterService = new GdprCustomScriptRegisterService(
                $this->context->controller,
                GdprCustomScript::getUnHookableScripts()
            );
            $return .= $scriptRegisterService->getInlineJs();
            $return .= $scriptRegisterService->getInlineCss();
        } catch (Exception $exception) {
    ...
    {
        $extraContent = "";
        $scripts = GdprCustomScript::getUnHookableScripts() + GdprProConfig::getModulesToUnload(true);
        
        $tabContentLink = Configuration::get(GdprProConfig::TAB_CONTENT_PAGE_ID);
        $tabContentPageId = Configuration::get(GdprProConfig::TAB_CONTENT_PAGE_ID);
        if (is_numeric($tabContentPageId)) {
            if (Validate::isLoadedObject($obj_cms = new CMS($tabContentPageId, Context::getContext()->language->id))) {
                $tabContentLink = Context::getContext()->link->getCMSLink((int)$obj_cms->id, $obj_cms->link_rewrite, Context::getContext()->language->id);
            }
        }

        if (file_exists(_PS_THEME_DIR_.'modules/gdprpro/views/templates/hook/template/')) {
            $v_tpl_path = _PS_THEME_DIR_.'modules/gdprpro/views/templates/hook/template/';
        } else {
            $v_tpl_path = _PS_MODULE_DIR_.'gdprpro/views/templates/hook/template/';
        }

        $this->context->smarty->assign(
            array(
                'modules_to_unload'          => $scripts,
                ...
                'rejectAllBtnShow'           => Configuration::get(GdprProConfig::REJECT_ALL_BTN_SHOW),
                'rejectAllBtnBgColor'        => Configuration::get(GdprProConfig::REJECT_ALL_BTN_BG_COLOR),
                'rejectAllBtnTextColor'      => Configuration::get(GdprProConfig::REJECT_ALL_BTN_TEXT_COLOR),
                ...
                'under18Enable'              => Configuration::get(GdprProConfig::UNDER_18_ENABLE),
                ...
                'popupOverlayBgColor'        => Configuration::get(GdprProConfig::POPUP_OVERLAY_BACKGROUND_COLOR),
                'popupOverlayBgOpacity'      => Configuration::get(GdprProConfig::POPUP_OVERLAY_BACKGROUND_OPACITY),
                'v_tpl_path'                 => $v_tpl_path,
    ...
    public function hookValidateCustomerFormFields()
    {
        ...
        if ($controller === 'IdentityController') {
            /**
             * @var $controller AuthControllerCore
             */
    ...
    public function hookActionCustomerAccountAdd($params)
    {
        if (isset($params['newCustomer'])) {
             ...
             */
            \PrestaChamps\GdprPro\Models\ActivityLogFactory::makeSignupFormConsent($customer);
            if ($customer->optin || $customer->newsletter) {
                \PrestaChamps\GdprPro\Models\ActivityLogFactory::makeNewsletterConsent($customer);
            }
        }
    }
}
