...
{/capture}
    <div class="gdpr-consent-tabs gdpr-consent-tabs-v2 {if isset($popupPosition)}popup-position-{$popupPosition}{/if}">
        ...
        <div class="div_accept_moreinfo">
            <span class="span_moreinfo">{l s='More info' mod='gdprpro'}</span>
            <button type="button" id="accept-all-gdpr"
                    style="background: {if isset($acceptAllBtnBgColor) && $acceptAllBtnBgColor}{$acceptAllBtnBgColor}{else}green{/if}; color:{if isset($acceptAllTextColor) && $acceptAllTextColor}{$acceptAllTextColor}{else}white{/if};">
                ...
                {foreach GdprPro::$cookieCategories as $cookie_Category}
                    ...
                    {if $cookies_InCategory > 0}
                        <div id="div_{$cookie_Category}" class="div_cookie_category">
								<span
                                    class="span-{$cookie_Category} {if $cookie_Category == 'necessary'}necessary{/if}">
                                    ...
                                    <table class="table table-bordered table-responsive">
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                ...
                                                <small
                                                    id="gdpr-check-all-modules">{l s='(Check all)' mod='gdprpro'}</small>
                                            </th>
                                        </tr>
                                        </thead>
                                        {foreach $modules_to_unload as $name => $module}
                                            {if isset($module.enabled) && $module.enabled == 1 && isset($module.category) && $module.category == $cookieCategory}
                                                ...
                                                <tr id="module_{md5($name)}">
                                                    <td class="td_name">
                                                        {$module.frontend_name[$langId]|default:""}
                                                    </td>
                                                    <td class="td_provider">{$module.provider[$langId]|default:""}</td>
                                                    <td class="td_description">
                                                        <span
                                                            class="tooltiptext">{$module.description[$langId]|default:""}</span>
                                                        <span
                                                            class="description">{$module.description[$langId]|default:""}</span>
                                                    </td>
                                                    <td class="td_expiry">{$module.expiry[$langId]|default:""}</td>
                                                    <td class="td_checkbox">
                                                        ...
                                                        checked disabled
                                                            {/if}>
                                                        <label
                                                            for="module-{$name}-chkbox"> {l s='Allow' mod='gdprpro'}</label>
                                                    </td>
                                                </tr>
                                            {/if}
                                        {/foreach}
