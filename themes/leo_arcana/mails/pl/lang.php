<?php

global $_LANGMAIL;
$_LANGMAIL = array();
$_LANGMAIL['Welcome!'] = 'Witaj !';
$_LANGMAIL['Process the payment of your order'] = 'Przetwórz płatność dla Twojego zamówienia';
$_LANGMAIL['Message from contact form'] = 'Wiadomość  od';
$_LANGMAIL['New credit slip regarding your order'] = 'Nowy \'kredyt\' dotyczący zamówienia';
$_LANGMAIL['The virtual product that you bought is available for download'] = 'Zakupiony przez Ciebie produkt wirtualny jest dostępny do pobrania';
$_LANGMAIL['Your new password'] = 'Twoje nowe hasło';
$_LANGMAIL['Fwd: Customer message'] = 'TR: Wiadomość od klienta';
$_LANGMAIL['Your guest account has been transformed into a customer account'] = 'Twoje konto gościa zostało przekształcone w konto klienta';
$_LANGMAIL['Package in transit'] = 'PegazShop.pl - Twoje zamówienie zostało wysłane';
$_LANGMAIL['Log: You have a new alert from your shop'] = 'Log: masz nowe ostrzeżenie z Twojego sklepu';
$_LANGMAIL['Your order has been changed'] = 'Zamówienie zmienione - pegazshop.pl';
$_LANGMAIL['Order confirmation'] = 'Potwierdzenie zamówienia';
$_LANGMAIL['Message from a customer'] = 'Wiadomość od klienta';
$_LANGMAIL['New message regarding your order'] = 'Nowa wiadomość dotycząca zamówienia';
$_LANGMAIL['Your order return status has changed'] = 'Nowy status zamówienia';
$_LANGMAIL['Password query confirmation'] = 'Potwierdzenie prośby o hasło';
$_LANGMAIL['An answer to your message is available #ct%1$s #tc%2$s'] = 'Dostępna jest odpowiedź na Twoją wiadomość #ct%1$s #tc%2$s';
$_LANGMAIL['New voucher for your order #%s'] = 'Twój nowy KOD RABATOWY na kolejne zakupy';
$_LANGMAIL['New voucher for your order %s'] = 'Twój nowy KOD RABATOWY na kolejne zakupy';
$_LANGMAIL['Newsletter confirmation'] = 'Potwierdzenie newslettera';
$_LANGMAIL['Email verification'] = 'E-mail weryfikacyjny';
$_LANGMAIL['Newsletter voucher'] = 'Twój KOD RABATOWY za rejestrację i zapisanie się do newslettera!';
$_LANGMAIL['Your wishlist\\\'s link'] = '';
$_LANGMAIL['Message from %1$s %2$s'] = 'Wiadomość od %1$s %2$s';
$_LANGMAIL['Your cart and your discount'] = '';
$_LANGMAIL['Thanks for your order'] = '';
$_LANGMAIL['You are one of our best customers'] = '';
$_LANGMAIL['We miss you'] = '';
$_LANGMAIL['Product available'] = 'Produkt ponownie dostępny - pegazshop.pl';
$_LANGMAIL['New order : #%d - %s'] = 'Nowa zamówienie - pegazshop.pl';
$_LANGMAIL['Stock coverage'] = '';
$_LANGMAIL['Product out of stock'] = 'Brak na stanie produktu - pegazshop.pl';
$_LANGMAIL['New return from order #%d - %s'] = '';
$_LANGMAIL['Congratulations!'] = '';
$_LANGMAIL['Referral Program'] = '';
$_LANGMAIL['%1$s sent you a link to %2$s'] = '%1$s wysłano Ci link do %2$s';

?>