		...
		{foreach name=wl from=$wishlists item=wishlist}
			{if $smarty.foreach.wl.first}
				<a class="wishlist_button_list" data-link="{$product.id_product|intval}" tabindex="0" data-toggle="popover" data-trigger="focus" title="{l s='Wishlist' mod='blockwishlist'}" data-placement="top">
					<i class="fa fa fa-heart-o"></i> <span>{l s='Add to wishlist' mod='blockwishlist'}</span>
				</a>
			...
			{/if}
		{foreachelse}
			<a href="#" id="wishlist_button_nopop" data-link="{$product.id_product|intval}" onclick="WishlistCart('wishlist_block_list', 'add', '{$id_product|intval}', $('#idCombination').val(), document.getElementById('quantity_wanted').value); return false;" title="{l s='Add to my wishlist' mod='blockwishlist'}">
				<i class="fa fa fa-heart-o"></i> <span>{l s='Add to wishlist' mod='blockwishlist'}</span>
			</a>
		{/foreach}
	</div>
{else}
	<div class="wishlist">
		<a class="addToWishlist wishlistProd_{$product.id_product|intval}" data-link="{$product.id_product|intval}" href="#" onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', false, 1); return false;" title="{l s='Add to my wishlist' mod='blockwishlist'}">
			<i class="fa fa-heart-o"></i> <span>{l s="Add to Wishlist" mod='blockwishlist'}</span>
		</a>	
	</div>
{/if}
