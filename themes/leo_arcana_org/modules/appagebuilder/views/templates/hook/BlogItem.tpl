...
<div class="blog-container" itemscope itemtype="https://schema.org/Blog">
    ...
    <div class="right-block">
		...
		<div class="blog-meta">
			...
			{if isset($formAtts.bleoblogs_scoun) && $formAtts.bleoblogs_scoun}
				<span class="nbcomment">
					{$blog.comment_count|intval} <span class="icon-comment">{l s='comment' mod='appagebuilder'}</span> 
				</span>
			{/if}
			
			{if isset($formAtts.bleoblogs_shits) && $formAtts.bleoblogs_shits}
				<span class="hits">
					{$blog.hits|intval}<span class="icon-hit">{l s='view' mod='appagebuilder'}</span> 
				</span>	
			{/if}
		</div>
		...
		<p class="blog-readmore">
			<a href="{$blog.link}{*full url can not escape*}" title="{$blog.title|escape:'html':'UTF-8'}">
				{l s='Read more' mod='appagebuilder'}<span class="fa fa-long-arrow-right"></span>
			</a>
		</p>
    </div>
</div>
