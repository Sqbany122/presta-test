...
{else}
									...
									{if $displayVouchers}
										...
										<div id="display_cart_vouchers">
											{foreach $displayVouchers as $voucher}
												{if $voucher.code != ''}<span class="voucher_name" data-code="{$voucher.code|escape:'html':'UTF-8'}">{$voucher.code|escape:'html':'UTF-8'}</span> - {/if}{$voucher.name} (kliknij w kod)<br>
											{/foreach}
										</div>
									{/if}
						...
						<tr class="cart_total_price">
							<td rowspan="{$rowspan_total}" colspan="2" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									...
									{/if}
									{if isset($cookie->id_customer) && $cookie->id_customer < 1033}
									<div id="display_cart_vouchers"> <span class="voucher_name" data-code="100STCLPG">100STCLPG</span> - 100 PLN dla dotychczasowych klientów PegazShop.pl (kliknij w kod)</div>
									{/if}
								{/if}
							</td>
	...
	<p class="cart_navigation clearfix">
		...
		{if !$opc}
			<a  href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}" class="button btn btn-outline standard-checkout button-medium pull-right" title="{l s='Proceed to checkout'}">
