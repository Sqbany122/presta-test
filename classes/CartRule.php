...
class CartRuleCore extends ObjectModel
{
    ...
    public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true, $check_carrier = true)
    {
		...
		if ($context->cart->id_customer > 1033 && $this->id == 279) {
			return (!$display_error) ? false : Tools::displayError('This voucher has already been used');
		}
        ...
        if ($this->minimum_amount && $check_carrier) {
            ...
            if ($cartTotal < $minimum_amount) {
				...
				//DODANE przeliczamy na nowo - nie uwzgędniamy produktów promocyjnych
...
//exit;
				foreach ($products as $product) {
                    ...
                    $cart_count_categories = Db::getInstance()->executeS('
							SELECT count(*) as count_prod
							FROM `'._DB_PREFIX_.'category_product` catp 
							WHERE (catp.id_category = 1440 OR catp.id_category = 1449) AND catp.id_product = '.(int)$product['id_product']);

					if($cart_count_categories[0]['count_prod'] < 1 && $product['price_without_reduction'] == $product['price_with_reduction']){
						
						$cartTotalNew += $product['price_without_reduction'] * (int)$product['quantity'];
					}
                }
				if ($cartTotalNew < $minimum_amount) {
					
					return (!$display_error) ? false : Tools::displayError('Nie osiągnąłeś minimalnej kwoty wymaganej, aby użyć tego kodu rabatowego. Bony podarunkowe oraz produkty z promocji i wyprzedaży są już przecenione i nie są uwzględnione do użycia kodu rabatowego. Kwota którą osiągnąłeś to: '.Tools::ps_round($cartTotalNew, _PS_PRICE_COMPUTE_PRECISION_).' zł.');
				}
			}
        }
