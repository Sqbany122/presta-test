...
<div class="form-group">
	<label class="control-label col-lg-3">Sposób liczenia rabatu</label>
	<div class="col-lg-9">
		<select name="exclude_discounts" class="form-control">
			<option value="0" {if $currentTab->getFieldValue($currentObject, 'exclude_discounts')|intval == 0}selected{/if}>
				Domyślne zachowanie
			</option>
			<option value="1" {if $currentTab->getFieldValue($currentObject, 'exclude_discounts')|intval == 1}selected{/if}>
				Zablokuj możliwośc użycia z przecenionymi produktami
			</option>
			<option value="2" {if $currentTab->getFieldValue($currentObject, 'exclude_discounts')|intval == 2}selected{/if}>
				Naliczaj rabat tylko dla produktów, które nie zostały przecenione
			</option>
		</select>
	</div>
</div>
