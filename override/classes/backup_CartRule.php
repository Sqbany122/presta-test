...
{
    /*
    * module: x13excludediscounts
    * date: 2020-11-05 13:31:44
    * version: 1.2.1
    */
    ...
    */
    public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true, $check_carrier = true)
    {
        ...
        foreach ($cart_rules as $cart_rule) {
            // Sprawdzenie warunków dla kodów typu "kod rabatowy" indywidualny
            if ((strpos($cart_rule['name'], 'Kod rabatowy') !== false && $cart_rule['quantity'] == 1)) { // || (strpos($cart_rule['name'], 'Bon podarunkowy') !== false && $cart_rule['quantity'] == 1)) {
                $excludeDiscount = true;
                break;
            }
            // Sprawdzenie warunków dla kodów typu "kod rabatowy" dla wszystkich
            if (strpos($cart_rule['name'], 'Kod rabatowy') !== false && $cart_rule['quantity'] > 1 && $this->quantity == 1 && strpos($this->name, 'Kod rabatowy') !== false) {
                $excludeDiscount = true;
                break;
            }
/*
            if (strpos($cart_rule['name'], 'Bon podarunkowy') !== false && $cart_rule['quantity'] > 1 && $this->quantity == 1 && strpos($this->name, 'Bon podarunkowy') !== false) {
                $excludeDiscount = true;
                break;
            }*/
        }		
        if ($excludeDiscount) {
            return !$display_error ? false : Tools::displayError('Nie możesz uzyć obydwóch kodów jednocześnie!.');
        }
		//
